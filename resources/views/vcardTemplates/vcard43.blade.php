    ...
    <main class="container">
        ...
        {{-- qr code --}}
                        ...
                        <ul class="text-decoration-none">
                            <li class="dropdown1 dropdown lang-list">
                                <a class="dropdown-toggle lang-head text-decoration-none" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                                    <i class="fa-solid fa-language me-2"></i>{{ getLanguage($vcard->default_language) }}</a>
                                <ul class="dropdown-menu start-0 lang-hover-list top-100 image-icon">
                                @foreach (getAllLanguageWithFullData() as $language)
                                    <li  class="{{ getLanguageIsoCode($vcard->default_language) == $language->iso_code ? 'active' : '' }}" >
            ...
            <div class="vcard-one__profile-details py-3 d-flex flex-column align-items-center justify-content-center">
                <div class="d-flex align-items-center gap-4 mt-4 w-100">
                    ...
                    <div class="main-heading text-center px-lg-3">
                        <h2 class="profile-name">{{ ucwords($vcard->job_title) }} {{ ucwords($vcard->company) }}</h2>
                        <h2 class="profile-designation">{{ ucwords($vcard->first_name . ' ' . $vcard->last_name) }}</h2>
                        <h2 class="profile-designation-2">@if ($vcard->occupation) ({{ ucwords($vcard->occupation) }})@endif</h2>
                    </div>
                    <div class="flex-grow-1">
                        <div class="divider border-white"></div>
                    </div>
                    ...
                    @if ($vcard->email)
                        <div class="col-sm-6 col-12 flex-column justify-content-center align-items-center">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <img src="{{asset('assets/img/vcard28/envelop.svg')}}">
                                </div>
            ...
            <section class="my-services py-5">
                <div class="container">
                    <h2 class="service-heading mb-5">{{ __('messages.vcard.our_service') }}</h2>
                        ...
                        <div class="col-sm-6">
                            ...
                            <img src="{{ $service->service_icon }}"  class="img-fluid" alt="{{ $service->name }}">
                                <div class="content">
                                    ...
                                    <p class="service-desc {{ \Illuminate\Support\Str::length($service->description) > 80 ? 'more' : '' }}">{!! $service->description !!}</p>
                                </div>
                            </div>
                        </div>
                  @endforeach
            ...
            @if (checkFeature('gallery') && $vcard->gallery->count())
              <section class="success gallary-section">
                <div class="container">
                    <h2 class="success-heading text-center mb-4">{{ __('messages.plan.gallery') }}</h2>
                    ...
                    @foreach ($vcard->gallery as $file)
                        ...
                        <a href="{{ $file->gallery_image }}" data-lightbox="gallery-images">
                            <img src="{{ $file->gallery_image }}" class="img-fluid w-100" alt="blogs-card-1">
                            </a>
                        </div>
                        @endif
                        @endforeach                        
         ...
         @endif
                         ...
                         <a href="{{ route('vcard.show-blog', [$vcard->url_alias, $blog->id]) }}">
                            <div class="d-flex justify-content-center align-item-center blogs-card card bg-transparent border-0 shadow-0">
                                <div class="d-flex justify-content-center blogs-img position-relative">
                                    <img src="{{ $blog->blog_icon }}" class="img-fluid" alt="{{ $blog->title }}">
                                </div>
            ...
            <section class="testimonial py-5">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <h4 class="testimonial-heading text-center mb-4">{{ __('messages.plan.testimonials') }}</h4>
                        </div>
                        <div class="col-12">
                            ...
                            @foreach ($vcard->testimonials as $testimonial)
                                <div class="col-sm-6">
                                    <div class="testimonial-card align-item-center mx-2" style="background-image: url('{{ asset('assets/img/vcard43/testimonial-background-img.png') }}')">
                                        <img src="{{ $testimonial->image_url }}" alt="{{ ucwords($testimonial->name) }}">
                                        <p class="mt-3 mb-3{{ \Illuminate\Support\Str::length($testimonial->description) > 80 ? 'more' : '' }}">{!! $testimonial->description !!} </p>
                                        <h3 class="mt-3 mb-3 lh-sm">{{ ucwords($testimonial->name) }}</h3>
                ...
                <div class="container pb-5">
                    <div class="row">
                        <div class="col-12 mb-4">
                            <h2 class="main-heading">{{ __('messages.make_appointments') }}</h2>
                        </div>
                        <div class="col-12">
                            <div class="appointment text-white">
                                <div class="mb-4 position-relative">
                                    ...
                                    {{ Form::text('date', null, ['class' => 'appoint-input mb-2 form-control rounded text-white p-3 bg-transparent', 'placeholder' => __('messages.form.pick_date'), 'id' => 'myID']) }}
                                    </div>

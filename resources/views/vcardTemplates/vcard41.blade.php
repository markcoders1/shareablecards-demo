    ...
    <main class="container">
        <section class="vcard-one main-content w-100 mx-auto position-relative">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="d-flex gap-3 align-items-center mb-3">
                                <img src="{{ $vcard->profile_url }}" alt="profile-img"
                                    class="img-fluid profile-img" />
                                <div class="main-heading text-center">
                                    <h1 class="profile-heading ">{{ ucwords($vcard->first_name . ' ' . $vcard->last_name) }}</h1>
                                    <h2 class="profile-name text-color-1">{{ ucwords($vcard->job_title) }} {{ ucwords($vcard->company) }}</h2>
                                    <h2 class="profile-designation text-white">@if ($vcard->occupation) ({{ ucwords($vcard->occupation) }})@endif</h2>
                                </div>
                            </div>
                            <p class='profile-description' style="font-size:12px; color:white;">{!! strip_tags($vcard->description) !!}</p>
                            <div class="social-icons d-flex justify-content-start gap-2 pt-4 flex-wrap">
            ...
            <section class="contact">
                <div class="container main-section5 py-4"
                    ...
                    @if ($vcard->email)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            <div class="icon-container gap-3">
                                ...
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.vcard.email_address') }}</h4>
                                    <a href="mailto:{{ $vcard->email }}"><p class="contact-heading2">{{ $vcard->email }}</p></a>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($vcard->alternative_email)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            <div class="icon-container gap-3">
                                <i class="fa-solid fa-message"></i>
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.vcard.alter_email_address') }}</h4>
                                    <a href="mailto:{{ $vcard->alternative_email }}"><p class="contact-heading2">{{ $vcard->alternative_email }}</p></a>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($vcard->phone)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            <div class="icon-container gap-3">
                                ...
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.vcard.mobile_number') }}</h4>
                                    <a href="tel:+{{ $vcard->region_code }}{{ $vcard->phone }}"><p class="contact-heading2">+{{ $vcard->region_code }}{{ $vcard->phone }}</p></a>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($vcard->alternative_phone)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            <div class="icon-container gap-3">
                                <i class="fa-solid fa-phone"></i>
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.vcard.alter_mobile_number') }}</h4>
                                    <a href="tel:+{{ $vcard->alternative_region_code }}{{ $vcard->alternative_phone }}"><p class="contact-heading2">+{{ $vcard->alternative_region_code }}{{ $vcard->alternative_phone }}</p></a>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($vcard->dob)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="22" viewBox="0 0 25 22" fill="none">
                                            ...
                                            <defs fill="#000000">
                                                <clipPath id="clip0_224_569" fill="#000000">
                                                    <rect width="25" height="22" fill="#000000"/>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.vcard.dob') }}</h4>
                                    <p class="contact-heading2">{{ $vcard->dob }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if ($vcard->location)
                        <div class="col-md-6 d-flex flex-column justify-content-center">
                            <div class="icon-container gap-3">
                                ...
                                <div>
                                    <h3 class="contact-heading1">{{ __('messages.setting.address') }}</h4>
                                    <p class="contact-heading2">{!! ucwords($vcard->location) !!}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                ...
                <div class="container" style="max-width: 540px;">
                    <div class="row">
                        <div class="gallery-heading text-center">
                            <h4>{{ __('messages.plan.gallery') }}</h4>
                        </div>
                ...
                <div class="contaier ">
                    ...
                    @foreach ($vcard->services as $service)
                        ...
                        <a href="{{ $service->service_url ?? 'javascript:void(0)' }}"
                            ...
                            <img src="{{ $service->service_icon }}" class="card-img rounded-0"
                                alt="{{ $service->name }}">
            ...
            <section id="blogs" class="blogs bg-color-1 pb-5">
                ...
                <div class="d-flex align-items-center gap-4 w-100">
                    <h2 class="main-heading2 text-center px-lg-3">{{ __('messages.feature.blog') }}</h2>
                </div>
                <div class="divider-verical mx-auto"></div>
                <div class="container">
                        ...
                        <a href="{{ route('vcard.show-blog', [$vcard->url_alias, $blog->id]) }}">
                            <div
                                ...
                                <div class="d-flex justify-content-center blogs-img position-relative">
                                    <img src="{{ $blog->blog_icon }}" class="img-fluid"
                                        alt="{{ $blog->title }}">
                                </div>
                                ...
                                <div class="row g-0">
                                    <div class="col-sm-4">
                                        <img src="{{ $testimonial->image_url }}"
                                            class="img-fluid h-100 w-100 object-fit-cover" alt="{{ ucwords($testimonial->name) }}">
                                    </div>
                                    <div class="col-sm-8 overlay-white"
                                        ...
                                        <div class="card-body px-lg-5 py-5">
                                            ...
                                            <h6 class="testimonial-name lh-sm">{{ ucwords($testimonial->name) }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <section id="appointment" class="appointment-wrapper bg-black">
                <div class="d-flex align-items-center py-5 w-100">
                    <h2 class="main-heading text-center pe-lg-3 ps-30 text-white">{{ __('messages.make_appointments') }}</h2>
                ...
                <div class="container pb-5">
                    <div class="row">
                        <div class="col-12">
                            <div class="appointment text-center text-white position-relative">
                                <div class="mb-4">
                                    <label for="myID" class="form-label">{{ __('messages.date') }}</label>
                                    {{ Form::text('date', null, ['class' => 'appoint-input mb-2 form-control rounded text-white p-3', 'placeholder' => __('messages.form.pick_date'), 'id' => 'myID']) }}
                                        <img src="{{ asset('assets/img/vcard41/appoinment-after.png') }}" class="appoinment-after">
                                </div>
                                <div class="mb-5">
                                    <label class="form-label">{{ __('messages.hour') }}</label>

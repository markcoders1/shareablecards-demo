...
<head>
    ...
    <style>
        .heading.text-center.margin-b-80px + .row{
           gap: 30px !important;
        }
...
document.addEventListener('DOMContentLoaded', function() {
		  ...
		  if (viewMoreButton) {
			viewMoreButton.addEventListener('click', function() {
			  var hiddenColumns = document.querySelectorAll('.hidden');
		...
		const heading = document.querySelector("h1.heading.textWhite.rubikBold");
		...
		const contactContentBox = document.getElementById("contactContentBox");
		let isAnimated  = false;

		arrowBtn.addEventListener("click", () => {
			if(!isAnimated){
				arrowBtn.style.animation="fadeOut 1s ease";
				ContactDescription.classList.remove("contactHidden");
				callToAction.classList.remove("contactHidden");
				ContactDescription.style.animation = "visible .5s";
				callToAction.style.animation = "visible .5s ease";
				heading.style.animation="slide 1s ease forwards";
				setTimeout(()=>{
					arrowBtn.classList.add("contactHidden");
				},1000)
    ...
    window.open("https://beremarkablelab.com/", "_blank");
		});
    </script>
    <script>

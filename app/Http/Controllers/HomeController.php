...
class HomeController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        $view = getSuperAdminSettingValue('is_front_page') ? view("front.home.$homePage", compact('plans', 'setting', 'features', 'testimonials', 'aboutUS', 'metas', 'faqs')) : redirect(route('home'));
    ...
    public function store(CreateContactRequest $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return response()->json(['error' => 'reCAPTCHA is required.'], 422);
        }
        ...
        if (!$responseBody->success) {
            \Log::error('reCAPTCHA validation failed: ', (array) $responseBody);
            return response()->json(['error' => 'reCAPTCHA validation failed.'], 422);
        }

        // If reCAPTCHA validation is successful, proceed with storing the contact message
        $input = $request->all();
